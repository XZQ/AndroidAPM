...
> 最后更新：2026-04-17 | 20 模块 | 67 源文件 | 25 测试类 | BUILD SUCCESSFUL
...
| **IO 监控** | 主线程 IO + 大 buffer + **Native PLT Hook** + FD 泄漏 + 吞吐量 + Closeable 泄漏 | File I/O 耗时统计 | - | 推荐 |
| **ANR** | **SIGQUIT 信号检测** + Watchdog 双重检测 + **traces.txt 解析** + 5 种原因分类 + 堆栈采样 + 去重 | Watchdog + 堆栈分析 | - | - |
| **内存泄漏** | WeakRef + Activity/Fragment/ViewModel 泄漏 + 引用链分析 | Activity/Fragment/Root | 监控 + 自动回收 | - |
| **OOM 预警** | OomMonitor + HprofDumper + HprofStripProcessor | Hprof Stripper | **fork 子进程 dump** + Strip | - |
...
| **WebView** | 页面加载耗时 + JS 执行耗时 + 白屏检测 | 页面加载耗时 | - | - |
...
5. **IO 多维检测**：FD 泄漏(/proc/self/fd) + 吞吐量统计 + PhantomReference Closeable 泄漏
...
| apm-core | com.didi.apm.core | Apm, ApmModule, ApmConfig, ApmDispatcher, ApmLogger, ApmContext, ProcessUtils | 7 |
...
| apm-uploader | com.didi.apm.uploader | ApmUploader, LogcatApmUploader, RetryingApmUploader | 3 |

### 3.2 功能模块层

| 模块 | 包名 | 核心类 | 源文件数 |
|------|------|--------|---------|
| apm-memory | com.didi.apm.memory | MemoryModule, MemorySampler, MemoryScheduler, MemorySnapshot, MemoryReporter, MemoryConfig | 15 |
| apm-crash | com.didi.apm.crash | CrashModule, CrashConfig, NativeCrashMonitor | 3 |
...
| apm-io | com.didi.apm.io | IoModule, NativeIoHook(PLT Hook+FD+吞吐量), IoConfig | 3 |
...
| apm-webview | com.didi.apm.webview | WebviewModule, WebviewConfig | 2 |
...
| 功能 | 完成度 | 已完成部分 | 待完成部分 |
|------|--------|-----------|-----------|
| Native Crash 信号处理 | 60% | Tombstone 解析（正则匹配 Signal/Backtrace/FaultAddr）+ 信号名映射 | 实际 signal handler 安装（需 JNI 库，推荐集成 Breakpad/Crashlytics NDK） |
| fork 子进程 dump | 40% | HprofDumper 异步 dump + 文件管理（最多5个）+ 自动触发 Strip | fork 子进程 dump（需 JNI 实现 fork+Debug.dumpHprofData，避免主线程 STW） |
| 多进程支持 | 30% | ProcessStrategy 枚举定义 + ProcessUtils 进程名判断 | 各模块多进程隔离逻辑、ContentProvider 初始化 |
| 引用链分析 | 30% | ActivityLeakDetector 反射分析 Context/View/Handler/Runnable/Thread 字段 | 完整 GC Root 引用链构建（对标 Shark/LeakCanary 2.0 的 HAHA 解析） |
| WebView 监控 | 50% | 页面加载耗时 + JS 执行耗时 + 白屏检测 | JS Bridge 性能监控 + 资源加载瀑布图 |

### 8.3 待开发

| 优先级 | 功能 | 说明 |
|--------|------|------|
| P0 | libapm-io.so JNI 库 | Native PLT Hook 需预编译 so 库（基于 xhook/bhook），当前仅 Java 层代理可用 |
| P0 | 上传通道实现 | LogcatApmUploader 仅为开发调试占位，需对接真实后端（HTTP/gRPC/自定义） |
| P1 | 零拷贝检测 | IoConfig.enableZeroCopyDetection 配置已预留，需实现检测逻辑 |
| P2 | 多进程完整支持 | 各模块适配 Service/ContentProvider 进程，数据隔离与合并 |
