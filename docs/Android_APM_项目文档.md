...
| 优先级 | 功能 | 说明 |
|--------|------|------|
| P0 | libapm-io.so JNI 库 | Native PLT Hook 需要预编译的 so 库（xhook/bhook） |
| P0 | Native Crash 信号处理 | NativeCrashMonitor 当前为占位，需实现 signal handler |
| P1 | Hprof 裁剪实现 | HprofStripProcessor 完整裁剪 primitive array |
| P1 | fork 子进程 dump | OOM 时 fork 子进程 dump hprof，避免主线程 STW |
| P2 | 多进程支持 | 当前为主进程设计，Service/ContentProvider 进程需适配 |
| P2 | 上传通道实现 | LogcatApmUploader 为占位，需对接真实后端（HTTP/gRPC） |

### 8.2 现有模块可增强

| 模块 | 增强方向 |
|------|---------|
| apm-memory | 引用链分析（对标 Shark/LeakCanary 2.0） |
| apm-network | HTTP 全链路 trace（DNS → TCP → TLS → Request → Response） |
| apm-webview | JS Bridge 性能监控 + 资源加载瀑布图 |
| apm-io | 零拷贝检测（当前 enableZeroCopyDetection 配置已预留） |
