...
- **仍需真机/外部系统**：预生产阶段按已固化的三 lane/六工件矩阵执行 24h/72h、功耗/热/长稳、弱网/断电、Native/IPC/OEM 实测，以及生产 Collector/幂等/查询告警、外部 Maven 与云端 runner。它们不阻塞当前客户端迭代；矩阵 plan、host tests、模拟器、microbenchmark 或短 smoke 也不能替代正式长稳验收。
