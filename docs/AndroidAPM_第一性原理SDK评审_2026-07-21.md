...
| 4 | 执行 24h 物理长稳 | 短 smoke 不能证明内存、磁盘、功耗、热和重启趋势；当前改用 Redmi，runner 已补 exact checkin UID fallback，但五分钟诊断值仍为零 | 原预算、无 override、物理设备、>=24h、>=24 次进程启动、离线积压与功耗证据全部通过；非功耗项可独立结论，完整门禁仍 fail closed |
...
- **物理设备**：Redmi/Xiaomi `22041216UC` 上 AndroidX 3/3 microbenchmark 与 JSON 预算通过；FPS observer 修复后两轮 smoke 以 CPU `12.928%`、`12.362%` 通过原 `20%` 上限。OnePlus `PLK110`（Android 16）历史 smoke 以 enabled CPU `6.161%` / `5.134%` 和 UID 功耗 `29.062` / `29.423 mAh/hour` 通过，但设备已不可用，不再作为下一轮依赖。当前 Redmi 的新电脑 smoke 以 CPU `12.201%` 通过；五分钟 10 events/second 诊断后 exact checkin UID `10217` computed power 仍为 `0`，因此不构成功耗接受。用户选择继续用该 Redmi 跑长稳；非功耗维度可单独判定，完整 24h 仍必须取得实际增长的 UID power 或外部仪器证据。MIUI session install 与历史 OnePlus `pm clear` 权限问题均和 SDK 预算结果分开记录。
